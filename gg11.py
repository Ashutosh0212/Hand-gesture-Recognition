...
def detect_object(img, detections, score_threshold, classNames, className, detectAllInstances):
    ...
    if detectAllInstances:
        ...
        if instance != None and className in classNames.values() and className == classNames[instance['classId']] and \
                instance['score'] > score_threshold:
...
def run_video_detection(input, mode, netModel, scoreThreshold, trackingThreshold, skipFrames, detectAllInstances):
    ...
    while showVideoStream:
        ...
        if totalFrames % skipFrames == 0:
            # run detection
            cvNet.setInput(
                cv.dnn.blobFromImage(img, 1.0 / 127.5, (300, 300), (127.5, 127.5, 127.5), swapRB=True, crop=False))
            ...
            if mode == 1:
                ...
                detect_object(img, detections[0, 0, :, :], scoreThreshold, netModel['classNames'],
                              currentClassDetecting, detectAllInstances)
            elif mode == 3:
                track_object(img, detections[0, 0, :, :], scoreThreshold, netModel['classNames'], currentClassDetecting,
                             trackingThreshold)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-f", "--skip_frames", type=int, default=1,
                        help="The number of frames to skip object detection")
    ...
    parser.add_argument("-t", "--tracking_threshold",
                        help="Tolerance (delta) between the object being detected and the position it is supposed to be in",
                        type=float, default=50)
    parser.add_argument("-a", "--detect_all_instances",
                        help="When in mode 2, whether or not to detect all instances. If false then detect the highest scored instance",
                        action="store_true")
    ...
    videoStreamThread = threading.Thread(target=run_video_detection,
                                         args=[args.input, args.detect_mode, netModels[args.net_model],
                                               args.score_threshold, args.tracking_threshold, args.skip_frames,
                                               args.detect_all_instances])
